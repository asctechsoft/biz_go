...
UserRole roleFromName(String? n) => UserRole.values
    .firstWhere((e) => e.name == n, orElse: () => UserRole.shipper);
...
StatusUi warehouseStatusUi(WarehouseStatus s) => switch (s) {
      ...
      WarehouseStatus.PREPARING =>
        const StatusUi('Đang chuẩn bị', AppColors.warning),
      WarehouseStatus.PREPARED =>
        const StatusUi('Đã chuẩn bị', AppColors.success),
      WarehouseStatus.PACKING =>
        const StatusUi('Đang đóng hàng', AppColors.warning),
...
StatusUi deliveryStatusUi(DeliveryStatus s) => switch (s) {
      DeliveryStatus.WAITING_ASSIGNMENT =>
        const StatusUi('Chờ xếp chuyến', AppColors.textSecondary),
      ...
      DeliveryStatus.DELIVERED =>
        const StatusUi('Giao thành công', AppColors.success),
...
StatusUi paymentStatusUi(PaymentStatus s) => switch (s) {
      ...
      PaymentStatus.PARTIAL => const StatusUi('Thanh toán 1 phần', AppColors.warning),
      PaymentStatus.PAID => const StatusUi('Đã thanh toán', AppColors.success),
      PaymentStatus.REFUNDED => const StatusUi('Đã hoàn tiền', AppColors.textSecondary),
