...
- **`UserRole`: owner (Chủ) · checker (Kiểm hàng) · sale (Sale)** — CHỈ 3 vai trò, KHÔNG còn `shipper`, cũng KHÔNG còn `warehouse` (Kiểm kho): vai trò đó đã **đổi tên + đổi quyền thành `sale`** (chỉ tạo đơn), việc kho gộp hẳn vào Kiểm hàng. `roleFromName` fallback về `sale` (quyền thấp nhất) → hồ sơ cũ còn `role: 'shipper'` **hoặc `role: 'warehouse'`** tự rơi về Sale, không cần script migrate Firestore.

Enum lưu Firestore bằng `.name`. Nhãn tiếng Việt + màu qua các hàm `*StatusUi()`.

## RBAC (`lib/core/permissions.dart` — class `Perm`)

- **owner:** toàn quyền — là người DUY NHẤT xem màn Tổng quan (`Perm.viewDashboard`) và đối soát giao hàng + thu tiền (`Perm.confirmDelivery`, `Perm.collectPayment`). **checker:** kho/đóng hàng/in phiếu, xem đơn, bấm **Xuất phát** (`Perm.startDelivery`). **sale:** CHỈ tạo đơn (`Perm.createOrder`) + xem đơn + in/chia sẻ phiếu — không kho, không tiền, không sửa/xoá/hủy đơn.
- **`Perm.createOrder` = owner + sale.** Sale tạo đơn xong là hết quyền đụng vào: `editOrder`/`deleteOrder`/`cancelOrder` vẫn chỉ owner. Ở màn tạo đơn, 2 ô **"Khách trả trước" + "Trạng thái thanh toán"** gate bằng `Perm.collectPayment` (chỉ owner) — `prepaid` cộng thẳng vào `customers.totalPaid` và trừ công nợ mà KHÔNG sinh doc `payments` nào đối chứng, nên nó là thu tiền trá hình. Sale tạo đơn thì đơn nằm ở `UNPAID`, owner thu sau bằng nút "Thu tiền".
- **Dialog cảnh báo "Khách đang nợ"** (bật khi chọn khách lúc tạo đơn / đặt lại đơn, `_warnDebtIfAny` trong màn tạo đơn) gate bằng `Perm.manageCustomers` — nó hiện **số tiền nợ** và nút "Nhắc nợ" mở thẳng màn chi tiết khách (nơi có nút **Thu công nợ**), nên Sale không được thấy. Nút đó `Navigator.push` thẳng chứ không qua go_router nên `Perm.canRoute` KHÔNG chặn hộ — phải tự gate.
...
- **Tiền chỉ owner + checker (`Perm.viewMoney`).** Đơn giá, thành tiền, tổng cộng, đã thu, còn thiếu, trạng thái thanh toán — ẩn hết với **sale**, cả trong app lẫn **trên phiếu in**. NGOẠI LỆ: **màn tạo đơn KHÔNG gate** (Sale phải thấy giá mới lên được đơn) — nên Sale thấy giá lúc soạn đơn của mình, nhưng danh sách/chi tiết đơn sau đó vẫn ẩn tiền như cũ. Cố ý, đừng "sửa cho nhất quán". KHÔNG có chỗ chặn chung: phải gate ở TỪNG nơi hiện tiền. Hiện đang gate ở `orders_screen` (thẻ đơn + tổng theo ngày + thanh lọc), `order_detail_screen` (dòng hàng, tổng cộng, `_PaymentCard`, chip trạng thái thanh toán), `delivery_hub_screen` (cả 3 tab — checker vào được cả 3, chỉ nút bấm bị chặn, nên phải truyền `showMoney` theo VAI TRÒ chứ đừng suy ra từ "tab này ai thao tác"), `quick_filter_screen` (bỏ ô lọc trạng thái thanh toán), `InvoiceScreen(showMoney:)` và `InvoicePdf.build(showMoney:)`. **NGOẠI LỆ có chủ ý:** dòng "Thanh toán: <trạng thái>" trên phiếu in nằm NGOÀI khối `showMoney` — mọi vai trò đều thấy, vì người giao/nhà xe cần biết đơn đã trả hay thu khi giao, mà nhãn trạng thái không lộ con số. Trong app thì chip trạng thái thanh toán vẫn bị ẩn như cũ. Thêm chỗ hiện `money()` mới thì nhớ gate.
- **`/dashboard` chỉ owner.** `Perm.home(role)` = tab đầu tiên, nên sale đăng nhập vào thẳng `/orders` (tab của sale chỉ có `/orders` + `/more`; `/more` với sale tự rút gọn còn hồ sơ + phiên bản + đăng xuất vì mọi tile khác đều gate theo quyền). Hộp thông báo `/notifications` trước đây CHỈ mở được từ màn Tổng quan → màn Đơn hàng có thêm chuông (`_NotifButton`) cho vai trò không thấy Tổng quan; bỏ nút đó đi là checker mất chỗ đọc thông báo "Đơn mới".
